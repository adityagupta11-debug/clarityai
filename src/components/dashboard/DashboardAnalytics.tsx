...
const BLUE = "#0050FF";
const PURPLE = "#7C3AED";
const SKILL_COLORS = [CYAN, PURPLE, BLUE];
...
function DashTooltip({
  ...
  return (
    <div className="rounded-lg border border-white/10 bg-[#0A0A0C]/90 px-3 py-2 shadow-xl backdrop-blur-xl">
      {label != null && label !== "" && (
        <p className="mb-0.5 text-[10px] uppercase tracking-widest text-white/40">{label}</p>
      )}
      <p className="text-sm font-semibold text-white">
        {payload[0]?.value}
        <span className="font-normal text-white/40">{suffix}</span>
      </p>
    </div>
  );
}
...
function ChartCard({
  ...
  return (
    <div className={cn(CARD, "flex flex-col p-5 transition-all duration-300 hover:border-white/10 hover:bg-white/[0.04]")}>
...
function Placeholder({ message }: { message: string }) {
  return (
    <div className="flex h-[150px] items-center justify-center px-4 text-center text-xs text-white/30">
      {message}
    </div>
  );
}
    ...
    <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
      ...
      <div className={cn(CARD, "group relative flex flex-col items-center overflow-hidden p-8 text-center")}>
        <div
          ...
          style={{ background: "radial-gradient(circle at 50% 30%, rgba(0,214,255,0.12), transparent 62%)" }}
        />

        <div className="relative flex flex-1 flex-col items-center justify-center">
          {/* Pulsating neon-blue microphone */}
          <Link
            href="/interview/new"
            aria-label="Start your first analysis"
            className="group/badge relative mb-7 flex h-40 w-40 items-center justify-center"
          >
            <div className="pointer-events-none absolute h-40 w-40 rounded-full bg-[#00D6FF]/20 blur-[70px]" />
            <div className="absolute h-40 w-40 rounded-full border border-white/5" />
            <div className="absolute h-28 w-28 rounded-full border border-[#00D6FF]/15" />
            <div className="absolute h-28 w-28 rounded-full border border-[#00D6FF]/30 animate-ping [animation-duration:3s]" />
            <div className="relative z-10 flex h-20 w-20 items-center justify-center rounded-2xl gradient-blue-cyan animate-float shadow-[0_0_45px_rgba(0,214,255,0.55)] transition-all duration-300 ease-out group-hover/badge:scale-105 group-hover/badge:shadow-[0_0_60px_rgba(0,214,255,0.75)]">
              <Mic className="h-9 w-9 text-white" />
            </div>
          </Link>

          <h3 className="text-2xl font-semibold tracking-tight text-white">AI Interview Coach</h3>
        ...
        <Link
          href="/interview/new"
          className="relative mt-7 inline-flex items-center gap-2 rounded-full gradient-blue-cyan px-7 h-12 text-sm font-semibold text-white shadow-[0_0_30px_rgba(0,214,255,0.4)] transition-all duration-300 ease-out hover:scale-105 hover:shadow-[0_0_45px_rgba(0,214,255,0.6)]"
        >
            ...
            {mounted ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={activity} margin={{ top: 6, right: 4, left: -22, bottom: 0 }}>
                  ...
                  <YAxis allowDecimals={false} width={26} tick={{ fill: "rgba(255,255,255,0.4)", fontSize: 11 }} axisLine={false} tickLine={false} />
                  <Tooltip cursor={{ fill: "rgba(255,255,255,0.04)" }} content={<DashTooltip suffix=" interviews" />} />
            ...
            {!mounted ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={scoreTrend} margin={{ top: 6, right: 6, left: -22, bottom: 0 }}>
                  ...
                  <YAxis domain={[0, 100]} width={26} tick={{ fill: "rgba(255,255,255,0.4)", fontSize: 11 }} axisLine={false} tickLine={false} />
                  <Tooltip cursor={{ stroke: "rgba(255,255,255,0.15)" }} content={<DashTooltip suffix=" / 100" />} />
                  <Area type="monotone" dataKey="score" stroke={CYAN} strokeWidth={2} fill="url(#dashArea)" dot={{ r: 2, fill: CYAN, strokeWidth: 0 }} activeDot={{ r: 4 }} />
                </AreaChart>
              </ResponsiveContainer>
            ) : (
              ...
              <div className="mt-1 flex items-center justify-center gap-3">
                {skills.map((s, i) => (
                  <div key={s.name} className="flex items-center gap-1.5">
                    <span className="h-2 w-2 rounded-full" style={{ background: SKILL_COLORS[i] }} />
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={skills} dataKey="value" nameKey="name" innerRadius="62%" outerRadius="88%" paddingAngle={3} stroke="none">
                    {skills.map((s, i) => (
                      <Cell key={s.name} fill={SKILL_COLORS[i]} />
                    ))}
                  </Pie>
